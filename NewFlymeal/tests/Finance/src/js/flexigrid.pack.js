...
(function($) {
	...
	$.addFlex = function(t, p) {
		...
		p = $.extend({ // apply default properties
			...
			title : false,
			idProperty : 'id',
			pagestat : '当前显示 {from}条 至 {to}条 总共记{total} 条记录',
			pagetext : '当前页',
			outof : '总页数',
			findtext : 'Find',
			...
			__mw : { // extendable middleware function holding object
				datacol : function(p, col, val) { // middleware for formatting
													// data columns
					var _col = (typeof p.datacol[col] == 'function') ? p.datacol[col](val) : val; // format
																									// column
																									// using
																									// function
					if (typeof p.datacol['*'] == 'function') { // if wildcard
																// function
																// exists
			...
			getGridClass : function(g) { // get the grid class, always
											// returns g
				return g;
			},
			datacol : {}, // datacol middleware object 'colkey':
							// function(colval) {}
		...
		var g = {
			...
			dragMove : function(e) {
				if (this.colresize) {// column resize
					...
					if (e.pageX > this.hset.right || e.pageX < this.hset.left || e.pageY > this.hset.bottom
				...
				if (p.dataType == 'json') {
						...
						function() {
							...
							if (typeof row.cell == 'undefined') {
								if (p.colModel[idx].render) {
									var hDom = p.colModel[idx].render(row[p.colModel[idx].name], row, idx, td, tr);
									if (hDom) {
										td.innerHTML = hDom;
									}
								} else {
								...
								td.innerHTML = p.__mw.datacol(p, $(this).attr('abbr'), iHTML); // use
																								// middleware
																								// datacol
																								// to
																								// format
					...
					$("rows row", data).each(function() {
						...
						$('thead tr:first th', g.hDiv).each(function() {
							...
							td.innerHTML = p.__mw.datacol(p, $(this).attr('abbr'), text); // use
																							// middleware
																							// datacol
																							// to
																							// format
			...
			autoResizeColumn : function(obj) {
				...
				var n = $('div', this.cDrag).index(obj), $th = $('th:visible div:eq(' + n + ')', this.hDiv), ol = parseInt(
						obj.style.left, 10), ow = $th.width(), nw = 0, nl = 0, $span = $('<span />');
				$('body').children(':first').before($span);
				$span.html($th.html());
		...
		if (p.colModel) { // create model if any
			...
			for (var i = 0; i < p.colModel.length; i++) {
				...
				if (cm) { // only use cm if its defined
					...
					if (cm.width) {
						if (widthRegExp.test(cm.width)) {
							var width = cm.width.substring(0, cm.width.length - 1)
							$(th).attr('width', bodyWidth * width * 0.01 - 12);
						} else {
		...
		$('thead tr:first th', g.hDiv).each(function() {
			...
			// if there isn't a default width, then the column headers don't
			// match
			// i'm sure there is a better way, but this at least stops it
			// failing
			if (this.width == '') {
				this.width = 100;
			}
		...
		if (p.colResize === true) {
			...
			if (cdcol !== null) {
				...
				g.cdpad = 0;
				g.cdpad += (isNaN(parseInt($('div', cdcol).css('borderLeftWidth'), 10)) ? 0 : parseInt($('div', cdcol)
						.css('borderLeftWidth'), 10));
				g.cdpad += (isNaN(parseInt($('div', cdcol).css('borderRightWidth'), 10)) ? 0 : parseInt($('div', cdcol)
						.css('borderRightWidth'), 10));
				g.cdpad += (isNaN(parseInt($('div', cdcol).css('paddingLeft'), 10)) ? 0 : parseInt($('div', cdcol).css(
						'paddingLeft'), 10));
				g.cdpad += (isNaN(parseInt($('div', cdcol).css('paddingRight'), 10)) ? 0 : parseInt($('div', cdcol)
						.css('paddingRight'), 10));
				g.cdpad += (isNaN(parseInt($(cdcol).css('borderLeftWidth'), 10)) ? 0 : parseInt($(cdcol).css(
						'borderLeftWidth'), 10));
				g.cdpad += (isNaN(parseInt($(cdcol).css('borderRightWidth'), 10)) ? 0 : parseInt($(cdcol).css(
						'borderRightWidth'), 10));
				g.cdpad += (isNaN(parseInt($(cdcol).css('paddingLeft'), 10)) ? 0 : parseInt(
						$(cdcol).css('paddingLeft'), 10));
				g.cdpad += (isNaN(parseInt($(cdcol).css('paddingRight'), 10)) ? 0 : parseInt($(cdcol).css(
						'paddingRight'), 10));
		...
		if (p.usepager) {
			...
			$(g.bDiv).after(g.pDiv);
			var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> </div> <div class="pGroup"><span class="pcontrol">'
					+ p.pagetext
					+ ' <input type="text" size="4" value="1" /> '
					+ p.outof
					+ ' <span> 1 </span></span></div> </div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> </div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> </div> <div class="pGroup"><span class="pPageStat"></span></div>';
			...
			if (p.useRp) {
				...
				$('.pDiv2', g.pDiv).prepend(
						"<div class='pGroup'><select name='rp'>" + opt
			...
			if (p.searchitems) {
				...
				$(g.sDiv).append(
						"<div class='sDiv2'>" + p.findtext + " <input type='text' value='" + p.query
								+ "' size='30' name='q' class='qsbox' /> " + " <select name='qtype'>" + sopt
								+ "</select></div>");
		...
		if ($('th', g.hDiv).length) {
			...
			$('th div', g.hDiv).each(
					function() {
						...
						$('tbody', g.nDiv).append(
								'<tr><td class="ndcol1"><input type="checkbox" ' + chk + ' class="togCol" value="' + cn
										+ '" /></td><td class="ndcol2">' + this.innerHTML + '</td></tr>');
						cn++;
					});
			if (browser.msie && browser.version < 7.0)
